...
class Envelope extends Array {
  ...
  parse( buffer, offset, length ) {
    ...
    var content = this.header.get( 'content-type' )
    var boundary = content && /^multipart\//i.test( content.type ) ?
      content.parameters.boundary : null
    var boundaryOffset = -1

    if( boundary != null ) {
      boundaryOffset = buffer.indexOf( '--' + boundary + '\r\n', eoh + EOH.length )
      if( boundaryOffset === -1 ) {
        var body = buffer.toString( 'ascii', eoh + EOH.length, offset + length )
        var match = /(?:^|\r\n)--([^\r\n]+?)(?:--)?\r\n/.exec( body )
        boundary = match ? match[1] : null
        boundaryOffset = boundary == null ? -1 :
          buffer.indexOf( '--' + boundary + '\r\n', eoh + EOH.length )
      }
    }
