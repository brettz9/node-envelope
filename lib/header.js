...
class Header extends Map {
  ...
  parse( buffer, offset, length ) {
    ...
    const headers = buffer.toString( 'utf8', offset, offset + length )
      .replace( /\r\n\s+/g, ' ' )
      .split( /\r\n/g )

    this.raw = headers.map(( line ) => {
      const eof = line.indexOf( ':' )
      if( eof === -1 ) {
        throw new Error( 'Invalid header – missing ":" delimiter' )
      }
      const field = line.slice( 0, eof )
      const value = line.slice( eof + 1 ).trim()
      return [ field, value ]
    })

    this.raw.forEach(( [fld, value] ) => {
      ...
      switch( field ) {
        case 'content-type':
          newValue = contentType.parse( value )
          break
        case 'content-disposition':
          newValue = contentDisposition.parse( value )
          break
        case 'bcc':
        case 'cc':
        case 'from':
        ...
        case 'sender':
        case 'to':
          newValue = value.split( ',' ).map( Contact.parse )
          break
        case 'subject':
          try {
            newValue = qp.decodeWord( value, true )
          } catch( e ) {
            newValue = value
          }
          break
